...
export const tokenXApiProxy = async (opts: Opts) => {
  ...
  opts.metricsStatusCodeCounter &&
    opts.metricsStatusCodeCounter.inc({ path: opts.prometheusPath, status: response.status });
  ...
  opts.logger.info(`Vellyket tokenXProxy-request mot ${opts.url}. Status: ${response.status}`);
  if (opts.noResponse) {
    return;
  }
  if (opts.rawResonse) {
...
export const tokenXApiStreamProxy = async (opts: AxiosOpts) => {
  ...
  opts.logger.info('content-type fra klient' + opts.req?.headers['content-type']);
  const requestId = randomUUID();
  try {
    const stopTimer = opts.metricsTimer
      ? opts.metricsTimer.startTimer({ path: opts.prometheusPath })
    ...
    stopTimer();
    opts.metricsStatusCodeCounter &&
      opts.metricsStatusCodeCounter.inc({ path: opts.prometheusPath, status: data.status });
